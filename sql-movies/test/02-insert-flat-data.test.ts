...
const insertMovies = (movies: Movie[]) => {
  return (
    ...
    movies.map(movie => 
      {
        const taglineValue = movie.tagline ? `'${escape(movie.tagline)}'` : 'NULL';
        return `('${movie.imdbId}', ${movie.popularity}, ${movie.budget}, ${movie.budgetAdjusted}, ${movie.revenue}, ${movie.revenueAdjusted}, '${escape(movie.originalTitle)}', '${escape(movie.homepage)}', ${taglineValue}, '${escape(movie.overview)}', ${movie.runtime}, '${escape(movie.releaseDate)}')`;
      })
      .join(",")
  );
};
